#-------------------------------------------------
    ...
    Function Write-Logfile()
    {
        ...
        $logOutput = "$timestamp $logentry" | Out-File $output -Append
    ...
    $logstoremove = Get-ChildItem -Path "$($Logpath)\*.*" -Include *.log | Where {$_.PSIsContainer -eq $false}
